...
const TimerSelector = ({ countDownConstant, setCountdownConstant, disabled, reset, difficulty, language, numberAddOn, symbolAddOn }) => {
  ...
  const handleSelect = (value) => {
    setCountdownConstant(value);
    reset(value, difficulty, language, numberAddOn, symbolAddOn, false);
    handleClose();
  };

  const timers = [
    { value: COUNT_DOWN_15, label: "15s" },
    { value: COUNT_DOWN_30, label: "30s" },
    { value: COUNT_DOWN_60, label: "60s" },
    { value: COUNT_DOWN_90, label: "90s" },
  ];

  return (
    <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
      <Tooltip title="Select time limit">
        <Button
          id="timer-select-button"
          aria-controls={open ? 'timer-menu' : undefined}
          aria-haspopup="true"
      ...
      >
        {timers.map((timer) => (
          <MenuItem 
            key={timer.value}
            selected={countDownConstant === timer.value}
            onClick={() => handleSelect(timer.value)}
          >
            {timer.label}
          </MenuItem>
        ))}
...
const TypeBox = ({
  ...
  useEffect(() => {
    ...
    if (wordSpanRefs[currWordIndex]) {
      ...
      if (scrollElement && status === "started") {
        ...
        {menuEnabled && typingGameMode && (
          <Box sx={{ mb: 2, display: 'flex', justifyContent: 'center', gap: 2, flexWrap: 'wrap' }}>
            <TimerSelector
              ...
              setCountdownConstant={setCountDownConstant}
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              <Button
                variant={language === ENGLISH_MODE ? "contained" : "outlined"}
                onClick={() => reset(countDownConstant, difficulty, ENGLISH_MODE, numberAddOn, symbolAddOn, false)}
                disabled={status === "started"}
                sx={{
                  borderRadius: "20px",
                  fontSize: "0.75rem",
                  borderColor: "rgba(255, 152, 0, 0.5)",
                  color: language === ENGLISH_MODE ? "#fff" : "#FF9800",
                  backgroundColor: language === ENGLISH_MODE ? "#FF9800" : "transparent",
                  '&:hover': {
                    backgroundColor: language === ENGLISH_MODE ? "#e68a00" : "rgba(255, 152, 0, 0.04)",
                    borderColor: "#FF9800",
                  }
                }}
              >
              ...
              <Button
                variant={language === TAGALOG_MODE ? "contained" : "outlined"}
                onClick={() => reset(countDownConstant, difficulty, TAGALOG_MODE, numberAddOn, symbolAddOn, false)}
                disabled={status === "started"}
                sx={{
                  borderRadius: "20px",
                  fontSize: "0.75rem",
                  borderColor: "rgba(255, 152, 0, 0.5)",
                  color: language === TAGALOG_MODE ? "#fff" : "#FF9800",
                  backgroundColor: language === TAGALOG_MODE ? "#FF9800" : "transparent",
                  '&:hover': {
                    backgroundColor: language === TAGALOG_MODE ? "#e68a00" : "rgba(255, 152, 0, 0.04)",
                    borderColor: "#FF9800",
                  }
                }}
              >
